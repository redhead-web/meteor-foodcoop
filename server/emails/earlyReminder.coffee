...
Meteor.methods 
  '/email/early-shopping-reminder': ->
    ...
    for product in products
      ...
      if likers.length > 0
        likedProducers = _.union(likedProducers, _.pluck(likers, '_id'))
      
      likingUsers = Meteor.users.find( 
        _id: $in: _.pluck(likers, '_id')
      , fields: 
          'emails.address': 1
          'profile.name': 1
      ).fetch()
    ...
    for recipient in likeRecipients
      ...
      result = Mailer.send
        ...
        subject: "Producers you like need to know if you'll be buying from them this week"
    ...
    for recipient in favouriteRecipients
      result = Mailer.send
        to: "#{recipient.profile.name} <#{recipient.emails[0].address}>"
        replyTo: "#{Meteor.settings.email.name} <#{Meteor.settings.email.address}>"
        subject: "One day left to shop for Products you like"
